use super::{
    ...
    stc::{AddUser, DelUser, PageUser, UpdUser, User, SaveUser},
};
use crate::{
    api::sys::user_role, 
    get_pool, res::error::MyErr, util::page::PageResult, AppResult, Res};
use salvo::prelude::*;
...
pub async fn page(req: &mut Request, res: &mut Response) -> AppResult<()> {
    ...
    let param = req.parse_json::<PageUser>().await.unwrap();
    let items = dao::sel_page(&mut tx, &param).await?;
    let total_count = dao::cnt_by_page(&mut tx, &param).await?;
    tx.commit().await?;
    Res::suc::<_>()
        .data(PageResult::<User>::new(&param.page, total_count, &items))
        .render(res);
    Ok(())
}
...
pub async fn add(req: &mut Request, res: &mut Response) -> AppResult<()> {
    let param = req.parse_json::<SaveUser>().await?;

    // verify if exists the same name
    let count = dao::cnt_by_name(get_pool(), &param.name.clone().unwrap(), None).await?;
    if count > 0 {
    ...
    let user_id = ins(&mut tx, &param).await?;

    /* insert data into user_role relationship table */
    if let Some(role_ids) = param.role_ids.clone() {
        let role_ids: &HashSet<String> = &role_ids.into_iter().map(|role_id|role_id).collect();
        user_role::dao::ins_user_role(&mut tx, &user_id, role_ids).await?;
    }
    ...
    let info = sel(get_pool(), &user_id).await?;
...
pub async fn edit(req: &mut Request, res: &mut Response) -> AppResult<()> {
    let param = req.parse_json::<UpdUser>().await.unwrap();
    ...
    // verify if exists the same name in the same parent id
    let count = dao::cnt_by_name(&mut tx, &param.user_name, Some(&param.id)).await?;
    if count > 0 {
        return MyErr::new().msg("name already exists").build();
    }
...
pub async fn remove_by_ids(req: &mut Request, res: &mut Response) -> AppResult<()> {
    let param = req.parse_json::<DelUser>().await.unwrap();
